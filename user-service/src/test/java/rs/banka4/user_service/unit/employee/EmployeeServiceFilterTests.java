...
public class EmployeeServiceFilterTests {
    ...
    void testGetAllEmployeesWithFilters(
        String firstName,
        String lastName,
        String email,
        String position
        ...
        when(employeeRepository.findAll(any(Specification.class), eq(pageRequest))).thenReturn(
            employeePage
        );

        // Act
        ResponseEntity<Page<EmployeeDto>> response =
            employeeService.getAll(firstName, lastName, email, position, pageRequest);
