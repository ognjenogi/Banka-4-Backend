...
public class ClientContactControllerTests {
    ...
    void testGetAllClientContacts() throws Exception {
        ...
        Mockito.when(clientContactService.getAllClientContacts(any(), any(PageRequest.class))).thenReturn(page);
    ...
    void testGetSpecificClientContact() throws Exception {
        ...
        ClientContactDto clientContactDto = ClientObjectMother.generateBasicClientContactDto();
        Mockito.when(clientContactService.getSpecificClientContact(any(), eq(id))).thenReturn(clientContactDto);
    ...
    void testUpdateClientContact() throws Exception {
        UUID id = UUID.randomUUID();
        ClientContactRequest clientContactRequest = ClientObjectMother.generateBasicClientContactRequest();
        Mockito.doNothing().when(clientContactService).updateClientContact(any(), eq(id), any(ClientContactRequest.class));
        mockMvc.perform(put("/client-contact/{id}", id)
    ...
    void testDeleteClientContact() throws Exception {
        UUID id = UUID.randomUUID();
        Mockito.doNothing().when(clientContactService).deleteClientContact(any(), eq(id));
        mockMvc.perform(delete("/client-contact/{id}", id)
                        .with(SecurityMockMvcRequestPostProcessors.csrf())
                        .header("Authorization", "Bearer dummyToken"))
                .andExpect(status().isOk());
    }
