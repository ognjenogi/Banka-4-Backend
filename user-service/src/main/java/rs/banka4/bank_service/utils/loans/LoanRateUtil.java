...
import rs.banka4.bank_service.domain.loan.db.BankMargin;
import rs.banka4.bank_service.domain.loan.db.LoanType;
import rs.banka4.bank_service.exceptions.loan.LoanTypeNotFound;
import rs.banka4.bank_service.repositories.BankMarginRepository;
...
public class LoanRateUtil {
    ...
    public BigDecimal calculateInterestRate(BigDecimal referenceValue, LoanType loanType) {
        BankMargin bankMargin =
            bankMarginRepository.findBankMarginByType(loanType)
                .orElseThrow(LoanTypeNotFound::new);
        return (referenceValue.add(bankMargin.getMargin())).divide(
            new BigDecimal(12),
    ...
    /**
     * This calculates the amount that needs to be paid per month For fixed rate loans it stays the
     * same barring any penalties for delayed payments, but for variable rate loans it should be
     * called in conjunction with calculateInterestRate monthly in order to calculate for the
     * installment
     */
    public BigDecimal calculateMonthly(
