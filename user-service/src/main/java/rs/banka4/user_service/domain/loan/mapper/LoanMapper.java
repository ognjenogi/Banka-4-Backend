...
public interface LoanMapper {
    ...
    @Mapping(target = "currency", expression = "java(loan.getAccount().getCurrency().getCode())")
    LoanInformationDto toDto(Loan loan);

    @Mapping(source = "loan.loanNumber", target = "loanNumber")
    @Mapping(target = "loanType", source = "type")
    @Mapping(target = "accountNumber", expression = "java(loanRequest.getAccount().getAccountNumber())")
    @Mapping(target = "currency", expression = "java(loanRequest.getAccount().getCurrency().getCode())")
