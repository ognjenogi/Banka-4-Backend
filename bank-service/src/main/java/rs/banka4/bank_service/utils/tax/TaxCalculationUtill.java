...
public class TaxCalculationUtill {
     ...
     * <p>
     * Converts any debt amounts not in RSD into RSD using the provided
     * {@link ExchangeRateService}, sums up all unpaid and yearly debts,
     * and returns them in a DTO.
     *
     ...
     *           <li><b>unpaidTaxThisMonth</b>: sum of {@link UserTaxDebts#getDebtAmount()}, all converted to RSD</li>
     ...
     * performs the currency conversion if needed, updates both account balances,
     * zeroes out the user's debt, and logs a {@link Transaction}.
     *
     * @param accountFrom the user's account from which the tax will be deducted;
     *                    must match an existing {@link UserTaxDebts#getAccount()}
    ...
    public void chargeTax(Account accountFrom, Account accountTo) {
        ...
        UserTaxDebts debt = userTaxDebtsRepository
            .findByAccount_AccountNumber(accountFrom.getAccountNumber())
            .orElseThrow(() -> new IllegalArgumentException("Account has no tax debt"));

        var rawDebt = debt.getDebtAmount();
        var amountToDebit = rawDebt;

        if (accountFrom.getAvailableBalance().compareTo(rawDebt) < 0) {
            throw new InsufficientFunds();
        }

        if (!accountFrom.getCurrency().equals(accountTo.getCurrency())) {
            amountToDebit = exchangeRateService.convertCurrency(
                rawDebt, accountFrom.getCurrency(), accountTo.getCurrency()
            );
        }

        accountFrom.setAvailableBalance(accountFrom.getAvailableBalance().subtract(rawDebt));
        accountFrom.setBalance(accountFrom.getBalance().subtract(rawDebt));

        accountTo.setAvailableBalance(accountTo.getAvailableBalance().add(amountToDebit));
        accountTo.setBalance(accountTo.getBalance().add(amountToDebit));

        debt.setDebtAmount(BigDecimal.ZERO);
        debt.setYearlyDebtAmount(debt.getYearlyDebtAmount().add(amountToDebit));
        userTaxDebtsRepository.save(debt);

        accountRepository.saveAll(List.of(accountFrom, accountTo));
        transactionRepository.save(buildTransfer(
            accountFrom,
            accountTo,
            rawDebt,
            amountToDebit
